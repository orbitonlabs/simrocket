...
public class BatchSupervisor {
    ...
    public static final double dt = 1.0d / 80000;

    public static final String prefix = "./simulations/";

    public static class Constraints {
        // 20 steps
        public double min_density = 1000;
        public double max_density = 3000;
    ...
    public static void multirun() throws InterruptedException, ExecutionException, IOException {
        ...
        bw.write("name,max_velocity,max_height,time_of_flight,initial_acceleration,propulsion_duration,terminal_velocity,pressure_chamber,rocket_mass,water_mass,firststage_column_height,water_column_height");
        bw.newLine();
                ...
                for(double mass = constraints.min_mass; mass <= constraints.max_mass; mass += constraints.mass_step) {
                    for(double height = constraints.min_height; height <= constraints.max_height; height += constraints.height_step) {
                        for(double packing = constraints.min_packing; packing <= constraints.max_packing; packing += constraints.packing_step) {
                            ...
                            Rocket rocket = new Rocket(name, density, pressure, mass, height, packing);
                ...
                for(double mass = constraints.min_mass; mass <= constraints.max_mass; mass += constraints.mass_step) {
                    for(double height = constraints.min_height; height <= constraints.max_height; height += constraints.height_step) {
                        for(double packing = constraints.min_packing; packing <= constraints.max_packing; packing += constraints.packing_step) {
                            ...
                            bw.write(simulate(rocket, false).reportString()+","+rocket.config());
                            bw.newLine();
                            index++;
                        }
                    }
    ...
    public static Analyzer simulate(Rocket rocket, boolean flightrecording) {
        ...
        config.addElement("Resolution: "+1/dt);
        ...
        write(analyzer.report(), prefix+rocket.name+"/"+rocket.name+".analysis.txt");
        return analyzer;
    }
